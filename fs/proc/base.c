...
#ifdef CONFIG_COMPAT
long proc_qos_ctrl_compat_ioctl(struct file *file, unsigned int cmd,
				unsigned long arg)
{
	return do_qos_ctrl_ioctl(
		QOS_IOCTL_ABI_ARM32, file, cmd,
...
static ssize_t proc_dmabuf_rss_hwm_write(struct file *file,
					 const char __user *buf, size_t count,
					 loff_t *offset)
{
	struct inode *inode = file_inode(file);
